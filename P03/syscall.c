...
struct Process_Info process_list[MAXTABLESIZE];
struct Kernel_Thread *thread_node =(&s_allThreadList)->head;
struct Kernel_Thread *run_queue[MAXTABLESIZE];
struct Kernel_Thread *idel_thread = CPUs[Get_CPU_ID()].idleThread; //smp.h
...
do{
i++;
run_queue[i]=Get_Next_Runnable(); //kthread.h
}while(run_queue[i] != idel_thread);


for(i=0;i< (int)state->ecx&&thread_node!=0; i++){

if(thread_node->userContext == 0) memcpy(process_list[i].name,thread_node->threadName,20);
else memcpy(process_list[i].name,thread_node->userContext->name,MAX_PROC_NAME_SZB); //user.h

process_list[i].pid = thread_node->pid;

if(thread_node->owner == 0) process_list[i].parent_pid = 0;
else process_list[i].parent_pid = thread_node->owner->pid;

process_list[i].priority = thread_node->priority;

if(thread_node->refCount != 2 && !(thread_node->alive)) process_list[i].status=STATUS_ZOMBIE;
else {
	if(process_list[i].pid == g_currentThreads[0]->pid) process_list[i].status=STATUS_RUNNABLE;
	else{
		process_list[i].status=STATUS_BLOCKED;
		for(int j=0;run_queue[j]!=0;j++){
			if(process_list[i].pid == run_queue[j]->pid) process_list[i].status = STATUS_RUNNABLE;
		}//for
	}//else
}//else


process_list[i].affinity=thread_node->affinity;
process_list[i].currCore=0;
process_list[i].totalTime=thread_node->totalTime;

thread_node = thread_node->nextAll_Thread_List;
}

if(!Copy_To_User(state->ebx,process_list,state->ecx * sizeof(struct Process_Info))) return -1;
