...
extern struct User_Context *Create_User_Context(ulong_t size) {
    ...
    // TODO("Allocate memory for the user context"); // user Context에 해당하는 메모리 할당
	...
	context->ldtSelector = Selector(KERNEL_PRIVILEGE, true, GDT_index);
...
int Load_User_Program(char *exeFileData, ulong_t exeFileLength
    ...
    ulong_t maxva = 0;
    unsigned numArgs;		// 2
    ulong_t argBlockSize;	//2
    ulong_t size, argBlockAddr;
    struct User_Context *userContext = 0;

    // 1 : TODO("Find maximum virtual address"); // 가상주소의 크기가 얼마인지 봐야함
...

	{
		...
		// 위치 : src/geekos/argblock.c
		// command를 전달하여 argument에 필요한 버퍼 크기와 argument 개수를 결정한다.
		// virtSize = Round_Up_To_Page(maxva) + Round_Up_To_Page(DEFAULT_USER_STACK_SIZE + argBlockSize);
		// 위치 : include/geekos/mem.h
	}

    // 2: TODO("Determine size required for argument block");
     ...
     * to run the process.
     */
    size = Round_Up_To_Page(maxva) + DEFAULT_USER_STACK_SIZE; // 이건 그냥 쓰고
    argBlockAddr = size;
    size += argBlockSize;
    ...
    // TODO("Load segment data into memory"); // 스폰에 있는거 구현 참조
	...
	// -memcpy(); // 실행 가능한 segment를 생성된 메모리에 복사한다.
	// -나머지 User_Context field를 채운다. (코드 진입점, 스택 주소, argument block 주소 등)
	// : Create_User_Context()에서 초기화된 부분을 설정한다.
	// - Format_Argument_Block(); // 위치 : src/geekos/argblock.c
	// 사용자 메모리의 argument block을 전달받은 command와 개수로 설정한다.


    // TODO("Format argument block"); 
	Format_Argument_Block((char*)((ulong_t)context->memory) + (ulong_t)context->argBlockAddr), numArgs, context->argBlockAddr, command);

    // TODO("Fill in code entry point"); // 진입점
