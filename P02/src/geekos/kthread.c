...
// 레지스터를 0으로 초기화하고, esi 레지스터를 인자값으로 대체하여 유저스택에 push
static void Push_General_Registers_esi(struct Kernel_Thread *thread,  ulong_t value ) {
	Push(thread, 0);
	Push(thread, 0);
	Push(thread, 0);
	Push(thread, 0);
...
{
  /* link user thread with its UserContext (PCB) */
  Attach_User_Context(kthread, userContext);
    
  /*
   * Setting up the stack for a user thread
   *  so it looks like the thread is returning
   *  from an interupt
   */
	ushort_t dsSelector = userContext->dsSelector;
	ushort_t csSelector = userContext->csSelector;

  Push(kthread, dsSelector);       /* Stack Data selector (data selector) */
  Push(kthread, userContext->stackPointerAddr); /* Stack Pointer ( end of data memory)  */
  Push(kthread, EFLAGS_IF);               /* Eflags (IF bit should be set so interupts are enabled) */
  Push(kthread, csSelector);       /* Text selector (text selector) */
  Push(kthread, userContext->entryAddr);        /* Program Counter (code entry point address) */
  Push(kthread, 0);                             /* Error Code (0) */
  Push(kthread, 0);                             /* Interrupt Number (0) */

  	// eax, ebx, edx, edx, esi, edi, ebp 레지스터의 값을 차례대로 
	// 유저 스택에 push한다. esi를 제외하고 모두 0으로 초기화하고, 
	// esi는 인자 블록 주소를 집어넣는다.
  ...
  Push(kthread, dsSelector);       /* DS register (data selector) */
  Push(kthread, dsSelector);       /* ES register (data selector) */
  Push(kthread, dsSelector);       /* FS register (data selector) */
  Push(kthread, dsSelector);       /* GS register (data selector) */
}
...
{
  /* create user thread */
  struct Kernel_Thread* ret = Create_Thread(PRIORITY_NORMAL, detached);

  //KASSERT(ret); /* if ret == 0 then a thread was not created */
  
	if ( ret != 0 ) {
  /* setup threads stack */
  Setup_User_Thread(ret, userContext);
  
  /* put thread in run queue */
  Make_Runnable_Atomic(ret);
  }
  return ret;
}
